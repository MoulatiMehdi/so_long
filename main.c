...
void draw_react(t_render * image,t_player * player,t_color color)
{
    ...
    {
        ft_image_putpixel(image->back,x,y + i, color);;
        ft_image_putpixel(image->back,x + player->width - 1,y + i, color);
        i++;
    }
    j = 0;
    while(j < player->width)
...
void ft_engine_player_coord(t_engine * engine)
{
    ...
    while(i < map->height)
    {
        ...
        while(j < map->width)
        {
            if(map->data[i][j] == 'P')
            {
                engine->player->x = WINDOW_WIDTH / 2 - map->width * 32 + j * 64 + 8;
                engine->player->y = WINDOW_HEIGHT / 2 - map->height * 32 + i * 64 - 8;
                break;
            }
            j++;
        }
